...
from net_utils import (
    safe_reply, safe_reply_sticker, safe_send, safe_photo,
    safe_document, safe_audio, safe_video, safe_edit, safe_effective_reply
)
...
async def cancel(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    await safe_reply(update, "Niente da fare, eh")
...
def whitelist(chat_id):
    return chat_id in ADMIN_WHITELIST
...
async def id_check(update: Update, command_is_start=False) -> None:
    ...
    if deactivate_the_bot:
        if not whitelist(chat_id):
            await safe_reply(update, '█▀█ ███ █▀█   ▞▚ █▀█ C ███ ▛▄ ▞▚ . . . \n █▀█ ███ █▀█   ☰   ▞▚ █▀█ C ███ ▛▄ ▞▚   █ █▄▄   █▀█▀█ ███ █▀█▀█ ☰ █▀█ ▀█▀ ███ . . .')
            raise UnauthorizedAccess("You are not authorized to use this bot.")
    elif command_is_start == False:
        with open(ENV_PATH+'/secret_player_data.json', 'r') as file:
            data = json.load(file)
        if str(chat_id) not in data.keys():
            await safe_reply(update, 'Non ti conosco...\nProva a usare /start...')
            raise UnauthorizedAccess("User did not press start")
        
async def manutenzione(update: Update) -> None:
    if update.callback_query:
        await safe_reply(update.callback_query.message, "⚠️🔧 Questa funzione è in manutenzione 🔧⚠️")
    elif update.message:
        await safe_reply(update, "⚠️🔧 Questa funzione è in manutenzione 🔧⚠️")
    else:
...
async def curse_player(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if random.random() > 0.95:
        if random.random() > 0.90:
            ...
            await safe_reply(update, "Eheheh... Stavo scherzando...")
        else:
            await safe_reply(update, random.choice(cursed_replies))
            await asyncio.sleep(1)
            if random.random() > 0.50:
                await safe_reply_sticker(update, random.choice(cursed_stikers))
        await asyncio.sleep(1)
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    replies = [
        ...
        "Che le s̵̡͕̥̜̞̏̋͝t̶̙̤̑e̴̦̲̳̟̿̃̔̊̈́̋̄̽͜l̵͔͓͉̬̼̮̘̏̈́́̑̉̓͘̕ĺ̸̨̩̝͜͝e̴̬̤̭̫͖̿͛̀̏̅̓͛̐ tremino e l'o̶̻̫̼̒͋̃̀̿̈͜s̵̯̳̗̋͌͛͠c̸̳͂̌͌͘ͅư̵͙̣͑̐̇̀͝͝r̶͙̝̽i̸̛͙̥͈̠̔̋͋t̵̢̝̻̀̄à̶̫̀̓̄͐̑͌̕ risuoni...",
        'Ghihihihi... Pronti per una burla?',
        "Ku ku ku... Stai cercando guai?",
        "Hehehe... Chiedi e sarò il tuo aiutante misterioso!",
        "Ku ku ku... Non temere! S̵͕̒͋͜͜ä̴̧̻͔̟̪̥̞̼́̈́̈́̀̃b̶̡̤͉̍͂̏̒̽͛̚͘l̸͎̙̾͑e̶̫̺͕̻̐y̵̧̨̱͈̰̜͍͗̀̽̿̋e̶̢̙̥͋̈́ è qui...",
    ...
    if route != None and route != 'Non_detta':
        ...
        if random.random() > 0.01:
            await safe_reply(update, random.choice(replies))
        else:
            await safe_reply(update, 'Non so davvero se dovrei aiutarti... lasciamolo decidere al caso, Croce!')
            await asyncio.sleep(2)
            await safe_reply(update, '🪙')
            await asyncio.sleep(3)
            await safe_reply(update, "P̷e̸c̶c̷a̷t̷o̴.̵.̵.̵ ̷C̵o̵s̷a̵ ̴d̴o̶v̷r̴e̴i̸ ̷f̸a̸r̴e̷ ̸c̶o̷n̴ ̷t̷e̷ ̴a̵d̷e̵s̵s̴o̴.̴.̶.̵")
    ...
    if route == None:
        ...
        if not skip_route_selection:
            ...
            await safe_reply(update, "Quindi, bando alle ciance e iniziamo! Per prima cosa, dimmi il percorso su cui ti trovi", reply_markup=reply_markup)
            return ROUTE_SELECTION
        else:
            await safe_reply(update, "Quindi, bando alle ciance e iniziamo!")
        ...
        reply_markup = InlineKeyboardMarkup(keyboard)
        await safe_reply(update, "Prima mi hai ignorato... dimmi il tuo percorso... Guarda che altrimenti non posso funzionare per bene...", reply_markup=reply_markup)
        return ROUTE_SELECTION
    return ConversationHandler.END
...
async def route_selection(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    reply_markup = InlineKeyboardMarkup(keyboard)
    await safe_edit(query, "Grazie dell'informazione... Vuoi anche aggiungere subito la tua squadra?", reply_markup=reply_markup)
    return ADDING_TEAM
...
async def adding_team(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if add_ == 'no_add_team':
        await safe_edit(query, "Va bene... Ricordati che potrai sempre farlo usando /team...")
    else:
...
async def bonus(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    await safe_reply(update, """Uno contro uno, eh?... Dimmi un po' chi sono i due pokemon.\n\n Scrivi "Pokemon1 Pokemon2"...""")
...
async def show_main_bonus_menu(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if context.user_data.get('bonus_fallito') == True:
        context.user_data['moltiplicatore'] = MULT
        context.user_data['bonus_pokemon'] = update.message.text
    elif 'bonus_pokemon' not in context.user_data.keys():
        context.user_data['moltiplicatore'] = MULT
        context.user_data['bonus_pokemon'] = update.message.text
    ...
    try:
        if np1.lower() != context.user_data.get('bonus_pokemon').split(" ")[0].lower():
            await safe_reply(update, text="Ho assunto che con "+context.user_data.get('bonus_pokemon').split(" ")[0]+" tu intendessi "+np1)
        if np2.lower() != context.user_data.get('bonus_pokemon').split(" ")[1].lower():
            await safe_reply(update, text="Ho assunto che con "+context.user_data.get('bonus_pokemon').split(" ")[1]+" tu intendessi "+np2)
    except:
        pass
    if text[0] == 'M':
        context.user_data['bonus_fallito'] = False
        ...
        if update.callback_query:
            ...
            await safe_edit(update.callback_query, text, reply_markup=reply_markup)
...
async def show_command_bonus(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    reply_markup = InlineKeyboardMarkup(keyboard)
    await safe_edit(query, "Seleziona un moltiplicatore:", reply_markup=reply_markup)
...
async def show_command_help(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if query.data == HELP_START:
        ...
        await safe_edit(query, "◉‿◉")
        for _ in range(random.randint(0, 6)):
            chat_id = query.message.chat_id
            await safe_send(context, chat_id, "◉‿◉")
            await asyncio.sleep(1)
        keyboard = [[InlineKeyboardButton("Indietro", callback_data=HELP_BACK)]]
        reply_markup = InlineKeyboardMarkup(keyboard)
        await safe_send(context, chat_id, "◉‿◉", reply_markup=reply_markup)
    ...
    if query.data != HELP_HELP:
        await safe_edit(query, text, parse_mode='MarkdownV2', reply_markup=reply_markup)
...
async def gym(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if has_a_team(chat_id):
        ...
        row = [InlineKeyboardButton("Testa un Pokémon", callback_data="gym_test")]
        keyboard.append(row)
        reply_markup = InlineKeyboardMarkup(keyboard)
        await safe_reply(update, "Scegli che palestra affrontare.", reply_markup=reply_markup)
    else:
        await safe_reply(update, "Non hai un team... come pensi di battere una palestra!")
    ...
    if len(parts) == 1:
        if poke_exist(old_pokemon_name):
            ...
            if old_pokemon_name.lower() != pokemon_name.lower():
                await safe_reply(update, f"Assumo che con {old_pokemon_name} intendessi {pokemon_name.capitalize()}...")
            pokemon_name = pokemon_name + ' 0'
        await safe_reply(update, f"Assumerò il livello di {old_pokemon_name} sia 0...")
        ...
        if not poke_exist(old_pokemon_name.split()[0]):
            ...
            if old_pokemon_name.split()[0].lower() != new_pokemon_name.lower():
                await safe_reply(update, f"Assumo che con {old_pokemon_name.split()[0]} intendessi {new_pokemon_name.capitalize()}...")
            pokemon_name = new_pokemon_name + ' ' + old_pokemon_name.split()[1]
        else:
            pokemon_name = old_pokemon_name
    else:
        await safe_reply(update, "Ma che hai scritto?? Segui le istruzioni... Riprova.")
        return CHOOSING_POKEMON_GYM
    try:
        ...
        reply_markup = InlineKeyboardMarkup(keyboard)
        await safe_reply(update, "Vuoi decidere quante palestre nel futuro vedere? O bastano 4?", reply_markup=reply_markup)
        return CHOOSING_GYM_COUNT
    except Exception as e:
        print('Errore in gym test:', e)
        await safe_reply(update, "Qualcosa è andato storto con i nomi dei pokemon... Riprova.")
        return CHOOSING_POKEMON_GYM
...
async def handle_gym_count_choice(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if query.data == "gggg_test_4":
        await safe_edit(query, "Attendi il prossimo messaggio...")
        chat_id = query.message.chat_id
        result = await poke_gym_test(str(chat_id), pokemon_name, int(livello), 4)
        await safe_send(context, chat_id, f"Ecco i risultati per {pokemon_name} al {livello}\n(4 palestre future):\n\n{format_gym_test(result)}")
        return ConversationHandler.END
    elif query.data == "gggg_test_custom":
        await safe_edit(query, "Quante palestre future vuoi vedere? Inserisci un numero.")
...
async def handle_custom_gym_count(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    try:
        ...
        await safe_reply(update, "Pensi di essere divertente?")
        return CHOOSING_CUSTOM_GYM_COUNT
    if gym_count <= 0:
        await safe_reply(update, "Ho detto nel futuro...")
        return CHOOSING_CUSTOM_GYM_COUNT

    await safe_reply(update, "Attendi il prossimo messaggio...")
    pokemon_name, livello = context.user_data.get('test_pokemon').split(" ")
    chat_id = update.effective_chat.id
    result = await poke_gym_test(str(chat_id), pokemon_name, int(livello), int(gym_count))
    await safe_reply(update, f"Ecco i risultati per {pokemon_name} al {livello}\n({gym_count} palestre future):\n\n{format_gym_test(result)}")
    return ConversationHandler.END
...
def change_secret_player_data_team(update: Update, index, pl, new_value):
    with open(ENV_PATH+'/secret_player_data.json', 'r') as file:
        data = json.load(file)
...
async def team_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if update.callback_query:
        await safe_edit(update.callback_query, "Ecco qui il messaggio per modificare il tuo team... Ricorda che lo puoi sempre chiamare usando /team", reply_markup=reply_markup)
    else:
        await safe_reply(update, "Ricordati che ora puoi mandarmi direttamente la card, non serve che ti impicci qui.\nIl comando /team non sarà mai deprecato, ma potrebbe sbagliare, specie sui livelli delle evoluzioni...")
        await safe_reply(update, "Questo è il tuo team... Non male.", reply_markup=reply_markup)
...
async def choose_pokemon(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if poke_exist(pokemon_name) == True:
        ...
        await safe_reply(update, f"Quindi il tuo nuovo Pokémon è {pokemon_name}... A che livello?")
        return CHOOSING_LEVEL
    else:
        await safe_reply(update, "Non conosco questo Pokémon... Riprova.")
...
async def choose_level(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    try:
        ...
        await safe_reply(update, f"Capito, capito... Livello {level}.")
        ...
        keyboard = create_team_keyboard(current_team)
        reply_markup = InlineKeyboardMarkup(keyboard)
        await safe_reply(update, "Questo è il tuo team aggiornato...", reply_markup=reply_markup)

        return ConversationHandler.END
    except ValueError:
        await safe_reply(update, "... Riprova.")
        return CHOOSING_LEVEL
    ...
    if action in ['minus', 'plus']:
        ...
        if current_team[index][0] is not None:
            if action == 'minus' and current_team[index][1] > 1:
                ...
                keyboard = create_team_keyboard(current_team)
                reply_markup = InlineKeyboardMarkup(keyboard)
                await safe_edit(query, reply_markup=reply_markup)
        ...
        reply_markup = InlineKeyboardMarkup(keyboard)
        await safe_edit(query, "Vuoi modificare questo Pokémon?", reply_markup=reply_markup)
        return CONFIRM_EDIT
    elif action == 'nolevel':
        pass
    elif action == 'end':
        await safe_edit(query, "Squadra modificata con successo!")
    return ConversationHandler.END
...
async def confirm_edit(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if query.data == "edit_confirm_yes":
        await safe_edit(query, "Dimmi il nome del Pokémon:")
        return CHOOSING_POKEMON
        ...
        keyboard = create_team_keyboard(current_team)
        reply_markup = InlineKeyboardMarkup(keyboard)
        await safe_edit(query, "Questo è il tuo team.", reply_markup=reply_markup)
        return ConversationHandler.END
...
async def ping_all_start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if whitelist(chat_id):
        await safe_reply(update, "Dimmi quale messaggio vuoi inviare a tutte le chats...")
        return WAITING_FOR_MESSAGE
    else:
        await safe_reply(update, "Ehehehe... Ma chi ti credi di essere...")
        return ConversationHandler.END
    ...
    with open(ENV_PATH+'/secret_player_data.json', 'r') as file:
        ...
        if update.message.text:
            ...
            for idd in accounts.keys():
                try:
                    await safe_send(context, idd, content)
                except:
            ...
            for idd in accounts.keys():
                try:
                    await safe_photo(context, idd, content)
                except:
                    print(f'ping_all to id : {idd} failed. (Probably no chat existing with the bot)')
            ...
            for idd in accounts.keys():
                try:
                    await safe_document(context, idd, content)
                except:
                    print(f'ping_all to id : {idd} failed. (Probably no chat existing with the bot)')
            ...
            for idd in accounts.keys():
                try:
                    await safe_audio(context, idd, content)
                except:
                    print(f'ping_all to id : {idd} failed. (Probably no chat existing with the bot)')
            ...
            for idd in accounts.keys():
                try:
                    await safe_video(context, idd, content)
                except:
                    print(f'ping_all to id : {idd} failed. (Probably no chat existing with the bot)')
        else:
            await safe_reply(update, "Nah... Questo non lo posso inviare... Per adesso...")
    return ConversationHandler.END
    ...
    if whitelist(chat_id):
        with open(ENV_PATH+'/secret_player_data.json', 'r') as file:
            ...
            for id in accounts.keys():
                await safe_reply(update, f'Id: {id}, username: {accounts[id]["username"]}, first_name: {accounts[id]["first_name"]}')
    else:
        ...
        await safe_reply(update, text, parse_mode='MarkdownV2')
...
async def dex_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        pass
    if not context.args:
        await safe_effective_reply(update, "Devi specificare un Pokémon... guarda /help...")
        return
    ...
    await safe_effective_reply(update, dex_entry)
...
async def cell_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    reply_markup = InlineKeyboardMarkup(keyboard)
    await safe_reply(update, "Scegli la cella", reply_markup=reply_markup)
...
async def get_custom_x(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    try:
        ...
        if x > 0:
            ...
            await safe_reply(update, 'Non posso guardare alle caselle passate. Dimmi un altro "x"')
            return WAITING_FOR_X
    except ValueError:
        await safe_reply(update, """Valgono solo "x" numerici. Dimmi un altro "x" """)
        return WAITING_FOR_X
...
async def lega_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if has_a_team(chat_id):
        ...
        await safe_reply(update, "Non hai un team... come sei arrivato alla lega!?!")
...
async def lega_button_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == BEST_POKEMON:
        ...
        text = text.replace(".", "\\.").replace("-", "\\-").replace("!", "\\!")
        await safe_edit(query, text, parse_mode='MarkdownV2')
    elif query.data == ONE_VS_TEAM:
        await safe_edit(query, "Dimmi il nome di un allenatore della lega...")
        context.user_data.clear()
        ...
        await manutenzione(update)
        await safe_edit(query, "Di quale Pokémon vuoi conoscere i counter?")
        context.user_data.clear()
...
async def lega_single_get_trainer(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if context.user_data['enemy_trainer'] in enemies.keys():
        await safe_reply(update, "Dimmi un Pokémon e un livello.\n\nScrivi \"Pokémon Livello\"... ")
        return READ_POKEMON
    else:
        await safe_reply(update, "Non lo conosco! Riprova.")
        return READ_TRAINER
...
async def counter_main(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        await safe_reply(update, 'Qualcosa è andato storto con poke_counters...')
        return ConversationHandler.END
...
async def lega_indizio_main(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    for i, [pokeee, powww] in enumerate(context.user_data['counter_team']):
        ...
        if powww > pokeee_bst:
            await safe_reply(update, "⚠️ WARNING: Secondo i miei calcoli, questa potenza non è possibile per il pokemon indicato (Troppo alta!)... Arrotondo al massimo possibile.")
        elif (await get_power(pokeee, pokeee_lvl_int) != powww):
            await safe_reply(update, "⚠️ WARNING: Secondo i miei calcoli, questa potenza non è possibile per il pokemon indicato... Arrotondo alla più vicina.")
...
async def lega_single_button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if query.data.startswith(MULT_PREFIX):
        ...
        await safe_edit(query, "Dimmi un altro Pokémon.\n\nScrivi \"Pokémon Livello\"... ")
        return READ_POKEMON
...
async def fight_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if has_lega_ended():
        await safe_reply(update, "La lega è finita... usa il comando /lega!")
        ...
        reply_markup = InlineKeyboardMarkup(keyboard)
        await safe_reply(update, "Dimmi il tipo di incontro:", reply_markup=reply_markup)
        return CHOOSE_FIGHT_TYPE

    else:
        if random.random() > 0.05:
            await safe_reply(update, "Non hai un team... contro chi pensi di combattere!")
        else:
            await safe_reply(update, "Che cazzo mi chiedi di combattere se non so che Pokémon hai.")
...
async def fight_type_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await safe_edit(query, "Inserisci una lista di Pokémon:")
        ...
        if poke_exist(p):
            ...
            if new_p.lower() != p.lower():
                if poke_exist(new_p) == False:
                    await safe_reply(update, f'Mhh... Non mi risulta nessun "{p}"... Riprova a dirmi la lista.')
                    return ENTER_POKEMONS
                else:
                    await safe_reply(update, f"Assumo che con {p} intendessi {new_p.capitalize()}...")
                    new_poke_list.append(new_p)
        
    poke_list = [p.lower() for p in new_poke_list]
    
    await safe_reply(update, "Attendi un attimo per l'immagine...")
    try:
        if context.user_data['fight_type'] == 'trainer':
            image_path, e_p = await poke_fight(str(update.effective_chat.id), True, poke_list)
            ...
            image_path, e_p = await poke_fight(str(update.effective_chat.id), True, poke_list, is_capopalestra=True)
            ...
            image_path, e_p = await poke_fight(str(update.effective_chat.id), False, poke_list)
            if len(poke_list) == 1:
        ...
        with open(image_path, 'rb') as image_file:
            await safe_photo(context, str(update.effective_chat.id), image_file, caption=cap)
    except Exception as e:
        print(f"Errore durante la lotta: {e} (99% dei casi, se è requested pokemon not found, controlla l'even_evo_file)")
        await safe_reply(update, "Qualcosa è andato storto... ti chiedo di ricominciare chiamando /fight.")
    return ConversationHandler.END
...
async def card_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    font, font_small, font_big = load_fonts('/GhoulFriAOE.ttf', font_size=50, font_size_small=30, font_size_big=70)
    ...
    with open(image_path, 'rb') as image_file:
        await safe_photo(context, chat_id, image_file, caption="Ecco la tua card aggiornata...")
...
async def add_meme(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    chat_id = update.effective_chat.id
    if whitelist(chat_id):
        await safe_reply(update, "Mandami l'immagine meme, con il nome come caption!")
...
async def answer_asking_the_meme(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if not photo:
        await safe_reply(update, "Non vedo nessuna immagine...")
        return ConversationHandler.END

    if not caption:
        await safe_reply(update, "Mi serve anche un nome come didascalia (caption)!")
        return WAITING_FOR_MEME
    ...
    try:
        ...
        await safe_reply(update, f"Salvato come {caption}.png!")
    except Exception as e:
        print("Error while saving meme:", e)
        await safe_reply(update, "Ma che roba mi hai mandato? Non riesco a leggere questa immagine...")
...
async def team_update(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if photo:
        try:
            ...
            await safe_reply(update, "👓 Ricevuto! Dammi un secondo... 👓")
            photo_file = photo[-1]
            file = await context.bot.get_file(photo_file.file_id)
            photo_bytes = await file.download_as_bytearray()
            pil_image = Image.open(BytesIO(photo_bytes))
            ...
            if secret_data == [None, None, None, None, None, None, None, None, None]:
                await safe_reply(update, "Non mi sembra una card... Che fai, mi mandi i meme?")
                return ConversationHandler.END
            else:
                message_text = 'Team aggiornato:\n\n'
                print_warning = 0
                print_error = 0
                for x, y in zip(secret_data, errors):
                    if x[0] != None:
                        ...
                        if y == '10':
                            message_text += " (⚠️)"
                            if print_warning == 0:
                                await safe_reply(update, "⚠️ WARNING: Non sono sicuro di star leggendo bene questo nome... Ma se ho ragione, non c'è alcun problema!")
                                print_warning = 1
                        if y == '11':
                            message_text += " (⚠️🔞)"
                            if print_warning == 0:
                                ...
                                print_warning = 1
                            if print_error == 0:
                                await safe_reply(update, "🔞 ERROR: Secondo i miei calcoli, la potenza di questo Pokèmon è diversa... Questo mi farà sicuramente sballare i conti, tienilo a mente!")
                                print_error = 1
                        if y == '01':
                            message_text += " (🔞)"
                            if print_error == 0:
                                await safe_reply(update, "🔞 ERROR: Secondo i miei calcoli, la potenza di questo Pokèmon è diversa... Questo mi farà sicuramente sballare i conti, tienilo a mente!")
                                print_error = 1
                        message_text += f' lvl {x[1]}\n'
                if 'Sableye' in message_text:
                    await safe_reply(update, 'Ottima scelta comunque... Non ti deluderò!')
                await safe_reply(update, message_text)
...
async def text_answer(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if "grazie" in text.lower() or "thx" in text.lower() or "thanks" in text.lower():
        await safe_reply(update, random.choice(answers))

    if "tank" in text.lower():
        await safe_reply(update, "Ma impara a scrivere bene...")
