...
async def text_answer(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if "grazie" in text.lower() or "thx" in text.lower() or "thanks" in text.lower():
        ...
        output_raw = chatbot(text,
                            max_new_tokens=50,
                            do_sample=True,
                            )[0]["generated_text"]
        output_clean = clean_and_truncate(output_raw, text)
        await message.reply_text(output_clean)
...
# --- Create pipeline with stable generation parameters ---
chatbot = pipeline(
    "text-generation",
    model=model,
    tokenizer=tokenizer,
    # parametri di generazione più stabili
    max_new_tokens=150,       # numero massimo di token generati
    temperature=0.4,          # bassa temperatura = meno casualità
    top_p=0.95,               # nucleus sampling leggermente restrittivo
    repetition_penalty=1.5    # evita ripetizioni
)